...
TEST(KeyboardTests, ButtonPressedNow_WithModifier_IsPressedWithThatModifier) {
	...
	keyboard.register_event_with_modifier(KEY, ButtonEvent::Down, 64);
	keyboard.update();

	EXPECT_EQ(keyboard.key_pressed(KEY), true);
	EXPECT_EQ(keyboard.key_pressed_with_modifier(KEY, 64), true);
}
...
TEST(KeyboardTests, ButtonPressedNow_WithModifier_IsNotPressedWithOtherModifier) {
	...
	EXPECT_EQ(keyboard.key_pressed(KEY), true);
	EXPECT_EQ(keyboard.key_pressed_now(KEY), true);
	EXPECT_EQ(keyboard.key_pressed_with_modifier(KEY, 128), false);
	EXPECT_EQ(keyboard.key_pressed_now_with_modifier(KEY, 128), false);
}
