...
namespace editor {
	...
	SceneWindow::SceneWindow() {
		...
		m_canvas = platform::add_canvas(1, 1);
		m_scene.grid_canvas = platform::add_canvas(GRID_SIZE * 2, GRID_SIZE * 2, platform::TextureWrapping::Repeat);
		...
		m_scene.canvas = platform::add_canvas(canvas_width, canvas_height);
		m_scene.zoom_index = 0;
	}

	SceneWindow::~SceneWindow() {
		platform::free_canvas(m_canvas);
		platform::free_canvas(m_scene.canvas);
		platform::free_canvas(m_scene.grid_canvas);
	}
	...
	void SceneWindow::update(
		...
		// Initialize scene view
		{
			...
			if (m_canvas.texture.size != input.monitor_size) {
				platform::free_canvas(m_canvas);
				m_canvas = platform::add_canvas((int)input.monitor_size.x, (int)input.monitor_size.y);
			}
	...
	static void render_scene_view(
		...
		/* Render grid */
		{
			...
			set_texture_filter(editor_scene.grid_canvas.texture, editor_scene.zoom_index < 0 ? platform::TextureFilter::Linear : platform::TextureFilter::Nearest);
	...
	void SceneWindow::render(
		...
		render_scene_view(m_scene, text_system, renderer);
		renderer->pop_draw_canvas();
