	...
	void EngineLibraryHotReloader::update(EngineLibrary* engine_library) {
		...
		if (m_hot_reload_timer.elapsed_ms() >= 1000) {
			...
			if (m_library_loader->library_file_has_been_modified()) {
				...
				*engine_library = core::util::unwrap(m_library_loader->load_library(m_library_name.c_str()), [&](LoadLibraryError error) {
		...
		if (m_rebuild_command_is_running) {
			if (core::util::future_has_value(m_rebuild_engine_future)) {
